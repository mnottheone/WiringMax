...
//#define SYSFS_PWM_ROOT		"/sys/class/pwm/pwmchip0/" not implementing PWM right now 
...
#endif
int sysfsGpioSet(int handle, unsigned int value)	//checked
{
	char set_value = 0;
...
int sysfsGpioGet(int handle)				//checked
{
	char get_value = 0;
...
int sysfsGpioExport(unsigned int gpio, char *path, unsigned int max_path) //checked 
{
	FILE *fp = NULL;
	int ret = 0;
	char export_value[3] = "";
	...
	if (path == NULL || max_path < sizeof(fs_path)){
      ...
      /*  // CHECK IT WHAT COULD BE VALUE OF GPIO //
	if (gpio > 99) {
...
int sysfsGpioDirection(unsigned int gpio, int output, int outval)
{
	FILE *fp = NULL;
	int ret = 0;
	int handle = PIN_EINVAL; // arduino.h(wiring_digital.h)  PIN_EINVAL = 0xFFFFFFFF
	char dir_value[5] = "";
	char fs_path[SYSFS_BUF] = "";
	 ...
	 */
	if (output) {					// if you have to give output
		handle = gpio2gpiohandle(gpio);		//arduino.h(wiring_digital.h) get the handle 
		if (PIN_EINVAL == handle) {		// if handle is 0xFFFFFFFF return it 
			ret = handle;
			goto end;
		}
		ret = sysfsGpioSet(handle, outval);    // otherwise set the gpio = to outval
	}

end:
	return ret;
}
#if 0 // drive is automatically controlled //
int sysfsGpioSetDrive(unsigned int gpio, unsigned int mode)	//checked // 3 modes remember when used
{
	FILE *fp = NULL;
	int ret = 0;
	char value[8] = "";
...
int sysfsGpioEdgeConfig(unsigned int gpio, int mode) // checked 
{
	FILE *fp = NULL;
	int ret = 0, idx;
	char * edge_state [] = {
...
/* echo 'output' > /sys/class/gpio/gpio'gpio'/level */
// mode - one of the modes defined for arudino IRQs HIGH "high", LOW "low"
int sysfsGpioLevelConfig(unsigned int gpio, int mode) // checked
{
	FILE *fp = NULL;
	int ret = 0, idx;
	char * level_state [] = {
